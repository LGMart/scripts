...
#-Metadata----------------------------------------------------#
#  Personal script for Kali Linux                             #
###############################################################
#  Command line arguments:                                    #
#    -thinkpad = Set br/abnt2 keyboard for Thinkpad laptop    #
...
if [[ "${EUID}" -ne 0 ]]; then
  ...
  echo -e " ${BLUE}[*]${RESET} ${BOLD}Kali Linux script${RESET}"
  sleep 3s
fi
...
##### Set static & protecting DNS name servers
...
#--- Use NordVPN DNS
echo -e 'nameserver 162.242.211.137\nnameserver 78.46.223.24' > "${file}"
#--- Use Google DNS
...
if [[ $(which gnome-shell) ]]; then
  ...
  xset s 0 0  # ERROR --> Invalid MIT-MAGIC-COOKIE-1 keyxset: unable to open display ":0.0"
  xset s off
...
file=/etc/apt/source.list; [ -e "${file}" ] && cp -n $file{,.bkup}
cat <<EOF > "${file}" \
...
#Chave para o Debian 8 (jessie) --> wget -q https://www.virtualbox.org/download/oracle_vbox_2016.asc -O- | sudo apt-key add -
...
#--- Configure keyboard layout <--------------------------- ESTA RODANDO MESMO SE NAO SETADO
if [[ -n "${ThinkpadKey}" ]]; then
  (( STAGE++ )); echo -e "\n\n ${GREEN}[+]${RESET} (${STAGE}/${TOTAL}) Setting ${GREEN}Thinkpad ABNT2${RESET} ~ keyboard layout {RESET})"
...
grep -q '^file:///home ' "${file}" 2>/dev/null \
  || echo 'file:///home Home' >> "${file}"
grep -q '^file:///opt ' "${file}" 2>/dev/null \
  || echo 'file:///opt /opt' >> "${file}"
grep -q '^file:///etc ' "${file}" 2>/dev/null \
  || echo 'file:///etc /etc' >> "${file}"
...
file=$(find ~/.mozilla/firefox/*.default*/ -maxdepth 1 -type f -name 'prefs.js' -print -quit) ### ERROR --> No such file or directory
[ -e "${file}" ] \
...
##### Setup firefox's plugins -------- ERROR -------- Couldn't find Firefox folder
...
${color white}CPU0$color: ${cpu cpu0}%  ${color white}CPU1$color: ${cpu cpu1}%  ${color white}CPU2$color: ${cpu cpu2}% ${color white}CPU3$color: ${cpu cpu3}%
...
${cpugraph cpu2 25,120 000000 white} ${alignr}${cpugraph cpu3 25,120 000000 white}
...
(( STAGE++ )); echo -e "\n\n ${GREEN}[+]${RESET} (${STAGE}/${TOTAL}) Installing ${GREEN}Rainbowcrack${RESET} ~ Rainbow Table Cracker"
apt -y -qq install rainbowcrack \
  || echo -e ' '${RED}'[!] Issue with apt install'${RESET} 1>&2


##### Install VLC
(( STAGE++ )); echo -e "\n\n ${GREEN}[+]${RESET} (${STAGE}/${TOTAL}) Installing ${GREEN}VLC${RESET}"
apt -y -qq install vlc \
  || echo -e ' '${RED}'[!] Issue with apt install'${RESET} 1>&2
#Enable VLC for root user
sed -i 's/geteuid/getppid/' /usr/bin/vlc

  
##### Install vokoscreen
(( STAGE++ )); echo -e "\n\n ${GREEN}[+]${RESET} (${STAGE}/${TOTAL}) Installing ${GREEN}Vokoscreen${RESET} ~ Screen Capture"
apt -y -qq install vokoscreen \
  || echo -e ' '${RED}'[!] Issue with apt install'${RESET} 1>&2

  
##### Install mediainfo
(( STAGE++ )); echo -e "\n\n ${GREEN}[+]${RESET} (${STAGE}/${TOTAL}) Installing ${GREEN}Mediainfo${RESET} ~ Bridge network interfaces"
apt -y -qq install mediainfo \
  || echo -e ' '${RED}'[!] Issue with apt install'${RESET} 1>&2
...
##### Install dvcs-ripper
(( STAGE++ )); echo -e "\n\n ${GREEN}[+]${RESET} (${STAGE}/${TOTAL}) Installing ${GREEN}dvcs-ripper${RESET}"
apt -y -qq install git \
  || echo -e ' '${RED}'[!] Issue with apt install'${RESET} 1>&2
git clone -q -b master https://github.com/kost/dvcs-ripper.git /opt/dvcs-ripper-git/ \
  || echo -e ' '${RED}'[!] Issue when git cloning'${RESET} 1>&2
pushd /opt/dvcs-ripper-git/ >/dev/null
git pull -q
popd >/dev/null


##### Install Fluxion
(( STAGE++ )); echo -e "\n\n ${GREEN}[+]${RESET} (${STAGE}/${TOTAL}) Installing ${GREEN}Fluxion${RESET} ~ MITM WPA attacks"
apt -y -qq install git dhcpd hostapd lighttpd php7.0-cgi \
  || echo -e ' '${RED}'[!] Issue with apt install'${RESET} 1>&2
git clone -q -b master https://github.com/FluxionNetwork/fluxion.git /opt/fluxion-git/ \
  || echo -e ' '${RED}'[!] Issue when git cloning'${RESET} 1>&2
pushd /opt/fluxion-git/ >/dev/null
git pull -q
popd >/dev/null  
...
#Enabling IP Forward
(( STAGE++ )); echo -e "\n\n ${GREEN}[+]${RESET} (${STAGE}/${TOTAL}) Enabling ${GREEN}IP Forward${RESET}"
echo 1 > /proc/sys/net/ipv4/ip_forward

# Fix possible audio issues
(( STAGE++ )); echo -e "\n\n ${GREEN}[+]${RESET} (${STAGE}/${TOTAL}) Fixing ${GREEN}possible Audio issues${RESET}"
apt -y -qq install alsa-utils \
  || echo -e ' '${RED}'[!] Issue with apt install'${RESET} 1>&2
